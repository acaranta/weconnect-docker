...
#        pprint(r)
#        print('Sending %s request to %s', r.request.method, r.url)
#        print('Parameters: %s', r.request.url)
#        print('Headers: %s', r.request.headers)
#        print('Response with code: %d', r.status_code)
...
#        print('History: %s', r.history)
    ...
    def get_xls_history(self):
        self.__check_dashboard()
        r = self.__command('/-/rts/export-trip-statistics/scope/all')
        #since last refuel :/-/rts/export-trip-statistics/scope/cyclic
        resp = {}
        resp['fname'] = re.findall('filename="(.+)"', r.headers.get('content-disposition'))[0]
